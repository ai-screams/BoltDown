...
- `sidebarStore.ts` — Sidebar UI state (open/close, width, isResizing, file tree data, recent files). Persists width to localStorage with 300ms debounce. Max 20 recent files. `loadRecentFiles()` uses `isRecentFile` type guard for runtime validation of localStorage JSON (guards against invalid/corrupted data). `loadParentDirectory()` uses atomic `set({rootPath, treeData})` to prevent UI flash, with `console.error()` logging in catch block for debugging. Exports `useSidebarStore` with `setResizing(boolean)` for cross-component drag coordination.
