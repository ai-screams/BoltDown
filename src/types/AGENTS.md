...
- `sidebar.ts` — `FileTreeNode` (hierarchical tree data), `RecentFile` (path + name + timestamp), `SidebarTab` (`'files' | 'recent' | 'outline-solid'`), `HeadingNode` (level, text, line).
...
- `SidebarTab` now includes `'outline-solid'` for the Outline Panel (heading navigation)
